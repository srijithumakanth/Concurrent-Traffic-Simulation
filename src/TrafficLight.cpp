...
void TrafficLight::waitForGreen()
{
    ...
    while (true)
    {
        auto phase = _trafficLightPhaseQueue.receive();
        if (phase == TrafficLightPhase::green)
            return;
        
        std::this_thread::sleep_for(std::chrono::milliseconds(1)); // To reduce load on the processor between calls
    }
}
...
void TrafficLight::cycleThroughPhases()
{
    ...
    while (true)
    {
        ...
        if (elapsedSeconds >= cycleDuration)
        {
            ...
            auto sendFuture = std::async(std::launch::async, &MessageQueue<TrafficLightPhase>::send, &_trafficLightPhaseQueue, std::move(newPhase));
